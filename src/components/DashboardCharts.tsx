...
import { TrendingUp, Users, Newspaper, Code, Building2 } from 'lucide-react'
...
export function DashboardCharts({ briefs }: DashboardChartsProps) {
  ...
  const signalDistribution = briefs.reduce((acc, brief) => {
    const category = brief.signalTag.toLowerCase().includes('hiring') ? 'Hiring' :
                    brief.signalTag.toLowerCase().includes('funding') ? 'Funding' :
                    brief.signalTag.toLowerCase().includes('growth') ? 'Growth' :
                    brief.signalTag.toLowerCase().includes('launch') ? 'Product' : 'Other'
